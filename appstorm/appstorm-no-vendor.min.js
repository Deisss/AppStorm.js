...
w--&&!a.isNull(document.getElementById(e.id).Content)?(a.timer.remove(x),d()):0>=w&&a.isFunction(f)&&f(b,408)},null,200)}},trace:function(){return e}}}();a.callback={};
...
1)};m.prototype.removeCallback=function(b){for(var c=h.length-1;0<=c;--c)if(h[c]===b){if(!0===m.prototype.isRunning()){for(var d=u.length-1;0<=d;--d)u[d]===b&&u.splice(d,1);a.console.warn("a.synchronizer.removeCallback : you should not add/remove callback when synchronizer is running",1)}h.splice(c,1)}};m.prototype.start=function(){if(0==u.length){for(var n=0,k=h.length;n<k;++n)u.push(h[n]);m.prototype.dispatch("a.callback.chainer.start",{});q={};0==u.length&&c()}n=u.shift();a.isFunction(n)&&n({success:c,
...
function(d,f){a.isFunction(b.load)&&(-1==e||0<=e&&a.state.__currentGeneratedId()===e)&&b.load(f);var g=c.getFullData()||{};if(a.isObject(d.data))for(var h in d.data)g[h]=d.data[h];c.setFullData(g);c.done()})},function(c){a.isFunction(b.postLoad)&&(-1==e||0<=e&&a.state.__currentGeneratedId()===e)?b.postLoad(c):c.done()});f.setSuccess(d);f.setFail(g);f.start()}function t(b,c){for(var e=new a.callback.synchronizer,f=b.length;f--;)a.contains(d,b[f].id)&&function(b){e.addCallback(function(c){m(b,function(){c.done()})})}(b[f]);
e.setSuccess(c);e.setFail(g);e.start()}var n=b.length,x=null;c="unload"===c?"unload":"load";d=d||[];var w=new a.callback.chainer;if("unload"===c)for(;n--;)w.addCallback(h(b[n]));else for(var v=0;v<n;)w.addCallback(h(b[v])),v++;w.setSuccess(f);w.setFail(g);w.start()};
