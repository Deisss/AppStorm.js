...
3);a.console.log(f,3);return f},validate:function(b){if(!a.isNull(c(b,"novalidate"))&&!1===this.skipNoValidate)return[];b=e(b);for(var l=[],m="number range text search url email password color checkbox hidden radio".split(" "),t=RegExp("^[a-z]{2,}:\\/\\/([a-z0-9\\/\\.\\-_~+;:&=]{2,})$","gi"),n=RegExp("^.{2,}@.*\\.[a-z0-9]{2,}$","gi"),x=RegExp("^#([a-f]{3}|[a-f]{6})$","gi"),w=b.length;w--;){var v=b[w],A=v.tagName.toLowerCase();if("select"!==A&&a.isNull(c(v,"formnovalidate"))){var s=c(v,"type"),p=d(v),
r=v.value,E=c(v,"required"),y=c(v,"pattern");c(v,"multiple");var D=c(v,"min"),z=c(v,"max"),B=c(v,"step"),D=a.isNull(D)||""===D?null:parseFloat(D),z=a.isNull(z)||""===z?null:parseFloat(z),B=a.isNull(B)||""===B?null:parseFloat(B);"input"!==A||a.contains(m,s)||a.isNull(s)?"number"!==s&&"range"!==s||a.isNumber(r)?"url"!==s||t.test(r)?"email"!==s||n.test(r)?"color"!==s||x.test(r)?null!==E&&a.contains(k,s)&&(""===r||a.isNull(r))?l.push(f(v,p,"required",r)):null===y||"textarea"!==A&&!a.contains(g,s)&&!a.isNull(s)||
RegExp(y).test(r)?null===D&&null==z&&null==B||!a.contains(h,s)||(A=parseFloat(r),null!==D&&A<D?l.push(f(v,p,"min",r)):null!==z&&A>z?l.push(f(v,p,"max",r)):null!==B&&0!==A%B&&l.push(f(v,p,"step",r))):l.push(f(v,p,"pattern",r)):l.push(f(v,p,null,r)):l.push(f(v,p,null,r)):l.push(f(v,p,null,r)):l.push(f(v,p,null,r)):a.console.warn("Type "+s+" for input "+p+" not recognized or not supported",3)}}a.console.log("a.form.validate: found error list:",3);a.console.log(l,3);return l},validateAndGet:function(b){b=
...
e[p]);else for(p in e)h=b(p),h=RegExp(h,"gi"),s=s.replace(h,e[p]);if(0<=s.indexOf("{{")&&0<=s.indexOf("}}"))for(f=f.match(/\{\{[a-z0-9\-_]+\}\}/gi)||[],p=0,r=f.length;p<r;++p){var h=b(f[p]),k=f[p].replace("{{","").replace("}}",""),k=a.isNull(m[k])?null:m[k];a.isNull(k)||(h=RegExp(h,"gi"),s=s.replace(h,k))}return s=s.replace(/\{\{[a-z\-_0-9]+\}\}/gi,"")}return d}function e(b,c){a.isString(c)||(c=u);try{return b.getAttribute(c)}catch(d){return null}}function f(b){a.isNull(b)&&(b=document);var f=[];
a.isNull(e(b))||f.push(b);var h=[];b.querySelectorAll?(h=b.querySelectorAll("*["+u+"]"),0===h.length&&(h=b.querySelectorAll("*"))):h=b.getElementsByTagName("*");b=0;for(var s=h.length;b<s;++b)a.isNull(e(h[b]))||f.push(h[b]);h=0;for(b=f.length;h<b;++h){var p=f[h],s=e(p);if(!a.isNull(s)){var r=c(g,s),k={};a.isNull(r)&&(r=s);for(var r=r.match(/\{\{[a-z0-9\-_]+\}\}/gi)||[],y=0,t=r.length;y<t;++y){var n=r[y].replace("{{","").replace("}}",""),B=e(p,u+"-"+n);a.isNull(B)?(B="undefined"===m[n]?null:m[n],a.isNull(B)||
(k[n]=B)):k[n]=B}r=d(s,k);s=e(p,q);if(!a.isNull(s))try{p[s]=r}catch(x){}else if(p)if("INPUT"!==p.nodeName||"submit"!==p.type&&"reset"!==p.type)if("INPUT"===p.nodeName)try{p.placeholder=r}catch(F){}else if("FIELDSET"===p.nodeName)p.title=r;else if(!a.isNull(p.text)&&document.all)p.text=r;else if(a.isNull(p.childNodes))p.innerHTML=r;else if(s=p.childNodes.length,k=document.createTextNode(r),"leave"===l){for(r=!1;s--;)y=p.childNodes[s],3===y.nodeType&&(r||(p.insertBefore(k,y),r=!0),p.removeChild(y));
r||p.appendChild(k)}else{for(;s--;)p.removeChild(p.firstChild);p.appendChild(k)}else p.value=r}}}var g="en",h=["en","fr","de","sp"],k={},l="leave",u="data-tr",q="data-tr-attr",m={},t=!a.isNull(a.storage)&&!0===a.storage.persistent.support;if(t){var n=a.storage.persistent.getItem("a_language_store_language"),x=a.storage.persistent.getItem("a_language_store_allowed");a.isString(n)&&(g=n,f(document));a.isArray(x)&&(h=x)}return{setBehavior:function(b){l="erase"===b?"erase":"leave"},getBehavior:function(){return l},
...
c?!1:!0})},loadById:function(b,f,k){var p=a.clone(t),r=a.page.event.hash.getHash(),m=a.clone(t);a.state.helper.tree.selectBranch(p,l,h(b));a.state.helper.tree.selectBranch(m,l,g(r));var y=a.state.helper.tree.flat(p,l),m=a.state.helper.tree.flat(m,l),m=d(m.concat(y)),p=e(p);k=a.isFunction(k)?k:function(){};a.console.log("a.state.loadById: element allowed to proceed change: "+m+" (does not mean all of them will be loaded)",3);a.state.helper.chainer("load",p,m,-1,k);a.isNull(f)&&(f=r);b={id:b,itemList:a.state.helper.tree.flat(y,
...
if(0<k.length)this.unloadById(c,function(){for(var b=k.length;b--;)a.state.loadById(c,k[b]);n=m;f()});else{var h=a.clone(t),y=a.page.event.hash.getHash();a.state.helper.tree.selectBranch(h,l,g(y));for(var y=a.state.helper.tree.flat(h,l),q=y.length;q--;)if(y[q].id===c){var z=e([y[q]]),w=a.state.helper.tree.flat(h,l,u);a.console.log("a.state.forceReloadById: element allowed to proceed change: "+w+" (does not mean all of them will be loaded/unloaded)",3);var x=b();a.state.helper.chainer("unload",z,w,
