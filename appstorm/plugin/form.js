...
a.form = (function() {
    ...
    */
    function __attr(el, attr) {
        return (!a.isNone(el.getAttribute)) ? el.getAttribute(attr) : null;
    };
    ...
    */
    function __getFieldList(dom) {
        ...
        while(i--) {
            ...
            if(!a.contains(typeList, at) && !a.isNone(at)) {
    ...
    return {
        ...
        */
        get : function(dom) {
            ...
            while(i--) {
                ...
                if(!a.isNone(input) && __attr(input, "type") === "radio") {
        ...
        */
        validate : function(dom) {
            ...
            if(!a.isNone(__attr(dom, "novalidate")) && this.skipNoValidate === false) {
                return [];
            }
            ...
            */
            ...
            while(i--) {
                ...
                if(tagName === "select" || !a.isNone(__attr(el, "formnovalidate"))) {
                    continue;
                }
                ...
                min  = (a.isNone(min) || min === "")   ? null : parseFloat(min);
                max  = (a.isNone(max) || max === "")   ? null : parseFloat(max);
                step = (a.isNone(step) || step === "") ? null : parseFloat(step);

                // Check input type does existing in allowed type list
                if(tagName === "input" && !a.contains(allowedTypes, type) && !a.isNone(type)) {
                ...
                // Required test
                if( required !== null && a.contains(typeRequiredList, type) && (value === "" || a.isNone(value)) ) {
                    errorList.push(__validateError(el, name, "required", value));
                    continue;
                }

                // Pattern test
                if( pattern !== null && (tagName === "textarea" || (a.contains(typePatternList, type)) || a.isNone(type)) ) {
