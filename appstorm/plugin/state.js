    ...
    return {
        ...
        */
        add : function(ctrl, callback) {
            ...
            while(!a.isNone(a.state.helper.tree.getItem(__root, __getTreeFiller, __getTreeIdTester(ctrl.id)))) {
...
a.state.helper.tree = {
    ...
    */
    isInBranch : function(tree, filler, tester) {
        ...
        return (a.isNone(item)) ? false : true;
    },
    ...
    */
    selectBranch : function(tree, filler, tester) {
        ...
        while(i--) {
            ...
            if(tester(tree[i]) === false && a.isNone(this.getItem(children, filler, tester))) {
                tree.splice(i, 1);
                continue;
            }
    ...
    */
    selectLevel : function(tree, filler, level) {
        ...
        // Test level now
        if(a.isNone(level) || isNaN(level) || level <= 0) {
            return tree;
        }
...
a.state.helper.chainer = function(type, path, allowed, id, callback) {
    ...
    */
    function __generateDefaultDataLoader(name, data, options, internal) {
        ...
        return function(result) {
            ...
            if(a.isNone(result.getData("data"))) {
                result.setData("data", {});
            }
    ...
    */
    function __generateLoader(state, callback) {
        ...
        if(!a.isNone(a.language) && translate.length > 0) {
...
a.state.helper.parameter = {
    ...
    */
    replace : function(str, param, custom) {
        custom = (!a.isNone(custom)) ? custom : "(" + param.regex + ")";
    ...
    extrapolate : function(str, hash, internal, replace) {
        ...
        if (a.isString(str) && str.indexOf("{{") >= 0 && str.indexOf("}}") >= 0) {
            ...
            while(ps--) {
                ...
                // Replacing hashtag
                if( (param.name === "hash" || param.name === "hashtag") && !a.isNone(param.value)) {
