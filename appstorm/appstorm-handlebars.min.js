...
a.page.template={__tmpl:{},get:function(c,b,d,f){var h=null;a.isObject(window.Mustache)?h=window.Mustache:a.isObject(window.Handlebars)&&(h=window.Handlebars);if(null==h)a.console.error("a.page.template.get: unable to find Mustache.JS or Handlebars.JS ! Can't proceed",1);else{var n=c.replace(/[^a-zA-Z0-9\\-]/g,"_"),l="a_tmpl_"+n+"_a",t=function(b,c,d){a.isFunction(b)&&(a.isNone(h.to_html)?(c=h.compile(a.page.template.__tmpl[c]),b(c(d))):b(h.to_html(a.page.template.__tmpl[c],d)))};a.isString(this.__tmpl[l])?(a.console.log("a.page.template.get: loading "+
...
h){a.page.template.__tmpl[l]||(a.page.template.__tmpl[l]=c);t(d,l,b)},{},f))}},htmlToDom:function(c){var b=document.createElement("div");b.innerHTML=c.replace(/^\s+|\s+$/g,"");c=b.childNodes;for(var b=[],d=0,f=c.length;d<f;++d){var h=c.item(d);!a.isObject(h)||"1"!==h.nodeType.toString()&&"3"!==h.nodeType.toString()||b.push(h)}return b},remove:function(c,b){for(;c.firstChild;)c.removeChild(c.firstChild);a.isFunction(b)&&b()},append:function(c,b,d){var f=this.htmlToDom(b);if(a.isObject(f))for(var h=
0,n=f.length;h<n;++h)c.appendChild(f[h]);a.isNone(a.language)||a.language.translate(c);a.isFunction(d)&&d(b)},replace:function(c,b,d){this.remove(c);this.append(c,b,d)}};
